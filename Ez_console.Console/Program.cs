...
ezConsole.WriteToLine($"This line is very long like {BC.red}very very {FC.yellow}long{BC.reset}{FC.reset} that is going to be ouside the window and the line below are going to be eaten! (or not)", 17, 2);
...
// End
if (Console.LargestWindowWidth + Console.LargestWindowHeight != 0 && Console.WindowHeight > 6)
{
    Console.SetCursorPosition(0, Math.Max(0, Console.WindowHeight - 6));
    Console.ResetColor();
}